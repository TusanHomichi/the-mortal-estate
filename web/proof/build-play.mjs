...
const presentation = process.argv[3] || "inspection";
if (!["inspection", "first-expedition"].includes(presentation)) throw new Error("Unknown play presentation build mode");
...
await build({ configFile: false, root: web,
  define: { "import.meta.env.TME_PLAY_PRESENTATION": JSON.stringify(presentation) }, build: { outDir: output, emptyOutDir: true,
