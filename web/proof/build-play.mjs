...
const presentation = process.argv[3] || "pixel-art";
if (!["inspection", "pixel-art"].includes(presentation)) throw new Error("Retired or unknown play presentation build mode; use pixel-art or explicit inspection");
...
await build({ configFile: false, root: web,
  ...
  plugins: [{ name: "pixel-product-boundary", generateBundle(_options,bundle) {
    if(presentation!=="pixel-art")return;
    for(const item of Object.values(bundle))if(item.type==="chunk")for(const id of Object.keys(item.modules)) {
      if(/node_modules\/three\/|\/play\/studyRenderer\.|\/authoritative\/renderer\./.test(id))throw new Error(`3D module reached the pixel product: ${id}`);
    }
