...
try{
 ...
 collectGraphicsErrors(page,errors);page.on('pageerror',e=>errors.push(String(e)));page.on('websocket',s=>{s.on('framesent',e=>{const m=JSON.parse(String(e.payload));if(m.kind==='command')commands.push(m);});s.on('framereceived',e=>{const m=JSON.parse(String(e.payload));if(m.frame)frame=m.frame;});});
 ...
 const point=await page.locator('#world-canvas').evaluate((node,id)=>{const p=JSON.parse(node.dataset.worldActorPoints).find(p=>p.id===id);if(!p)throw Error("Player body missing");const b=node.getBoundingClientRect();return {x:b.left+p.px*b.width,y:b.top+p.py*b.height};},frame.observer_actor_id);
 ...
 assert(lost,'context-loss capability required');await page.getByRole('alert').filter({hasText:'World graphics were lost'}).waitFor();
 assert.equal(await page.locator('#world-canvas').getAttribute('data-world-view'),null);
 assert.equal(await page.locator('#world-canvas').getAttribute('data-presentation'),'world-3d');
