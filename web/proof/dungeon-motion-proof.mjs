...
try{
 ...
 async function move(x,y,name){
  await ready();const before=commands.length,p=await worldCellPoint(page,{x,y});await page.mouse.click(p.x,p.y);
 ...
 await move(24,9,'walk');await attack('jumpkick',['flying_kick']);await move(25,8,'engage');await attack('fight',['jab_left','jab_right','uppercut_right','hook_left'],'motion_fist');
