...
revision: 1
status: Historical checkpoint narrative through September 10; superseded by the current genesis checkpoint.
public_safe: true
summary: Earlier presentation, gameplay and deployment checkpoints retained as dated navigation and evidence.
---

# Checkpoint history through September 10

This History record preserves the earlier checkpoint narrative extracted from
[the genesis ledger](genesis-ledger.md#current-checkpoint) during the repository
audit. It is not a current dispatch or deployment manifest. Words such as
"current", "latest" and "next" below describe each earlier checkpoint; subsequent
records and maintained owners supersede them. Read this only when the task needs
that history. The live four-floor integration is recorded in the
[live-dungeon execution record](2026-09-10-live-dungeons.md).
