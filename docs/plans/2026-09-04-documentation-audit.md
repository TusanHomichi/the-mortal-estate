...
| Browser operating instructions were mixed with Godot environment controls | `web/vite.config.ts`, `web/src/presets.ts`, `web/proof/serve.mjs`, walk and capture scripts | [Browser operation](../browser-client.md#operation-and-proof) documents dev-server-only packet serving, URL controls, and optional proof inputs. A build is not a packaged private asset set. |
